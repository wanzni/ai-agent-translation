...
public interface DocumentTranslationRepository extends JpaRepository<DocumentTranslation, Long> {
     ...
     * @return 总下载次数
     */
    @Query("SELECT SUM(dt.downloadCount) FROM DocumentTranslation dt WHERE dt.status = cn.net.susan.ai.translation.enums.ProcessingStatusEnum.COMPLETED")
     ...
     * @return 字符数总和
     */
    @Query("SELECT SUM(dt.characterCount) FROM DocumentTranslation dt WHERE dt.status = cn.net.susan.ai.translation.enums.ProcessingStatusEnum.COMPLETED")
     ...
     * @return 平均处理时间
     */
    @Query("SELECT AVG(dt.processingTime) FROM DocumentTranslation dt WHERE dt.userId = :userId AND dt.processingTime IS NOT NULL AND dt.status = cn.net.susan.ai.translation.enums.ProcessingStatusEnum.COMPLETED")
     ...
     * @return 字符数总和
     */
    @Query("SELECT SUM(dt.characterCount) FROM DocumentTranslation dt WHERE dt.userId = :userId AND dt.status = cn.net.susan.ai.translation.enums.ProcessingStatusEnum.COMPLETED")
     ...
     * @return 过期的翻译任务列表
     */
    @Query("SELECT dt FROM DocumentTranslation dt WHERE dt.status IN (cn.net.susan.ai.translation.enums.ProcessingStatusEnum.PENDING, cn.net.susan.ai.translation.enums.ProcessingStatusEnum.PROCESSING) AND dt.createdAt < :beforeTime")
