...
/**
 * XLS 翻译器：遍历工作簿的文本单元格进行批量翻译，保留单元格样式。
 */
@Slf4j
public class XlsFileTranslator implements DocumentFileTranslator {
    ...
    public byte[] translate(byte[] sourceBytes,
        ...
        try (ByteArrayInputStream bais = new ByteArrayInputStream(sourceBytes);
            ...
            progressCallback.accept(55, "并行翻译中，共" + requests.size() + "个单元格");
